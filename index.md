...
- **Premium status** — A local cache indicating whether the premium feature has been unlocked via Google Play
...
- Is **never** automatically transmitted or shared
...
| Permission | Purpose |
|---|---|
| **RECORD_AUDIO** | Used only when you enable the optional hands-free sound-gesture feature. Audio is processed entirely on-device in real time and is never recorded, stored, or transmitted. |
| **VIBRATE** | Provides haptic feedback for timer events and gesture confirmations. |
| **FOREGROUND_SERVICE** | Keeps your timer running accurately when the App is in the background. |
| **POST_NOTIFICATIONS** | Displays notifications for timer progress and completion while the App is in the background. |

### Microphone Usage

The microphone is used **only** when you explicitly enable the sound-gesture control feature (disabled by default). When active:

- Audio is analyzed on-device in real time to detect snap and clap gestures
- No audio is recorded, stored, or transmitted
- Processing stops immediately when the feature is disabled or the App is closed
